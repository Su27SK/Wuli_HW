...
vector<vector<int>> PG::getMinCost(vector<int> nodes, int &mincost, int time_threshold)
{
	double start, stop;
	start = clock();
//	cout<<endl;
//	cout<<"--------------------------In PG--------------------"<<endl;
	...
	do{
		...
		{
			curinfo=cur[chosen];
			xin=setnewbrgroup(curinfo.nodelist);
			lastcur=cur;
		}
		else
		{
			...
			curinfo=cur[chosen];			
			cout<<"in setn"<<endl;
			xin=setngroup(curinfo.nodelist);
		}
		stop = clock();
...
vector<branchInfo> PG::setnewbrgroup(vector<int> node)
{		
	...
	for(size_t i=0;i<node.size();i++)
	{
		...
		for(size_t j=0;j<insect.size();j++)	
		{
			...
			if(!hastested(newx)) 
			{
				...
				{
					group.push_back(br);
	//					cout<<"1i="<<i<<"\tj="<<j<<"\t"<<br.funValue<<endl;
				}
			}
			newx=node;
			...
			{	
				...
				{
					group.push_back(br);
		//			cout<<"2i="<<i<<"\tj="<<j<<"\t"<<br.funValue<<endl;
				}	
			}		
		}
	}
...
vector<branchInfo> PG::setngroup(vector<int> node)
{		
	vector<branchInfo> group;
	vector<int> newx;
	vector<int> curn;
